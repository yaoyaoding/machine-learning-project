...
https://github.com/geek-ai/MAgent/blob/master/python/magent/builtin/tf_model/dqn.py
to make it compatible with OJ submission
"""
...
class Net:
    def __init__(self, env, handle, name='dqn',
    ...
    def save(self, dir_name, epoch):
        ...
        saver.save(self.sess, os.path.join(dir_name, ('tfdqn_' + "_%d") % epoch))


    def load(self, dir_name):
        model_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, self.name)
        saver = tf.train.Saver(model_vars)
        saver.restore(self.sess, os.path.join(dir_name, ('tfdqn_7')))
